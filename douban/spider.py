...
def getData(baseurl):
    ...
    for i in range(0, 10):          # 调用获取页面信息的函数，10次
        ...
        for item in soup.find_all('div', class_="item"):                 # 查找符合要求的字符串，形成列表
            ...
            data = []        # 保存一部电影的所有信息
            item = str(item) # 转换成字符串，为使用re做准备
            
            # 影片详情链接
            link = re.findall(findLink, item)[0]        # re库用来通过正则表式查找指定的字符串
            data.append(link)                           # 添加链接
            
            imgSrc = re.findall(findImgSrc, item)[0]
            data.append(imgSrc)                         # 添加图片
                                                        
            titles = re.findall(findTittle, item)       # 片名可能只有一个中文名，没有英文名
            if(len(titles) == 2):
                ctitle = titles[0]
                data.append(ctitle)
            ...
            bd = re.sub('/', " ", bd)                   # 替换/
            data.append(bd.strip())                     # 去掉后面的空格

            datalist.append(data)                       # 把出处理好的一部电影信息放入datalist
